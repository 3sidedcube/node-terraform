...
variable "db_engine" {
  ...
  validation {
    condition     = contains(["aurora-postgresql", "aurora-mysql", "mysql"], var.input_parameter)
    error_message = "Allowed values for input_parameter are \"aurora-postgresql\", \"aurora-mysql\", or \"mysql\"."
  }
}
...
variable "db_engine_mode" {
  ...
  validation {
    condition     = contains(["multimaster", "parallelquery", "provisioned", "serverless"], var.input_parameter)
    error_message = "Allowed values for input_parameter are \"multimaster\", \"parallelquery\", \"provisioned\", or \"serverless\"."
  }
}
